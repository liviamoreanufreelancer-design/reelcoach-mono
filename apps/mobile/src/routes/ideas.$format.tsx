...
function Ideas() {
  ...
  useEffect(() => {
    setProfId(getProfessionId());
  }, []);

  const safe: Format = (VALID as string[]).includes(format) ? (format as Format) : "before-after";
  ...
  return (
    <PhoneShell>
      <CinematicBg src={heroImg} blur overlay={0.8} kenBurns={false} />
            ...
            return (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <p className="text-white/65 text-[13px] italic mt-1.5 leading-snug">„{s.hook}"</p>
                  {s.description && (
    ...
    >
      <span className="flex gap-[2px]">
        {[0, 1, 2].map((i) => (
          <span
            key={i}
            className={`w-[5px] h-[5px] rounded-full ${i < dots ? "bg-current" : "bg-current/20"}`}
          />
        ))}
      </span>
      {meta.short}
