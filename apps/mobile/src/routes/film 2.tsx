...
export const Route = createFileRoute("/film 2")({
  component: Film,
});
...
function StoryboardScreen({
  ...
  const template = getTemplate(scenario.id);
...
function prettyPattern(p: ShotPatternId): string {
...
function PreShotScreen({
  ...
  // Pattern label for the eyebrow badge — uppercase, prettified.
  const patternLabel = scene.tag ?? prettyPattern((scene.patternId ?? "before") as ShotPatternId);
